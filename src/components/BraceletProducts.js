...
const BraceletProducts = ({ products = [], handleAddToCart, onOpenQuickView }) => {
    // Filter products of category Bracelet
    const braceletProducts = products.filter(product => {
        const cat = product.category ? product.category.toLowerCase() : '';
        const name = product.name ? product.name.toLowerCase() : '';
        return cat === 'bracelet' || name.includes('bracelet');
    });
    ...
    return (
        <section className="bracelet-products">
            <div className="container">
                ...
                {braceletProducts.length === 0 && <p style={{ textAlign: 'center', color: 'var(--color-text-muted)' }}>No bracelet products found.</p>}
            </div>
        </section>
    );
};
