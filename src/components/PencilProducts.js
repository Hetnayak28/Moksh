...
const PencilProducts = ({ products = [], handleAddToCart, onOpenQuickView }) => {
    // Filter products of category Pencil
    const pencilProducts = products.filter(product => {
        const cat = product.category ? product.category.toLowerCase() : '';
        const name = product.name ? product.name.toLowerCase() : '';
        return cat === 'pencil' || name.includes('pencil');
    });
        ...
        <section className="pencil-products">
            <div className="container">
                ...
                <div className="product-grid">
                    {pencilProducts.map((product, index) => {
                ...
                {pencilProducts.length === 0 && <p style={{ textAlign: 'center', color: 'var(--color-text-muted)' }}>No pencil products found.</p>}
            </div>
        </section>
    );
};
